# Hi there, I’m Abass Ibrahim 👋

**IT Support Specialist (Oil & Gas) • Full-Stack Web Developer • Web3 & Bot Creator • UI/UX + Graphic Designer**  
📍 Lagos, Nigeria

---

## 👨‍💻 About me
I’m a detail-oriented IT Support Specialist and software developer who loves building clean, fast, and user-friendly products.  
I work mostly with **React, Next.js, Tailwind CSS, and Node.js**, and I also build **Web3 tools and automation/bots**.  
I care about **clarity, maintainable code, great UI/UX, and delivering real results**.

---

## 🧰 Tech I use

**Languages**
- JavaScript • TypeScript • HTML • CSS

**Frameworks / Libraries**
- React • Next.js • Tailwind CSS • Node.js

**Tools**
- Git • GitHub • VS Code • Figma • Adobe (Photoshop/Illustrator/InDesign)

---

## ✅ What I’m good at
- Troubleshooting, IT support, and keeping systems running smoothly  
- Building responsive, modern UIs with React/Next.js + Tailwind  
- Turning designs into clean, reusable components  
- UI/UX + visual design (Figma + Adobe tools)  
- Automation & bots that save time and improve workflows  

---

## 📊 GitHub stats
![GitHub stats](https://github-readme-stats.vercel.app/api?username=Lingz450&show_icons=true)
![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=Lingz450&layout=compact)

---

## 🤝 Contact
- LinkedIn: (https://www.linkedin.com/in/abass-ibrahim-6a5795194/)
- Email: jesselingard990@gmail.com
- Portfolio: (add your link)
